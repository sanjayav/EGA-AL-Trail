...
const TONE_CLASSES: Record<BadgeTone, string> = {
  ...
  accent:
    'bg-[var(--color-gold-soft,var(--color-accent-soft))] text-[var(--color-gold-deep,var(--color-accent))]',
}
...
export function Badge({ tone = 'neutral', children, className }: BadgeProps) {
  return (
    <span
      className={cn(
        'inline-flex h-5 items-center gap-1 rounded-[var(--radius-sm)] px-2 text-[11px] font-medium',
        'tabular',
        TONE_CLASSES[tone],
        className,
      )}
